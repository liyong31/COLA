...
std::vector<spot::twa_graph_ptr> decomposer::run() {
  ...
  struct pair_compare {
    // reverse order
    bool operator()(std::pair<std::set<unsigned>, unsigned> scc_i,
                    std::pair<std::set<unsigned>, unsigned> scc_j) const {
  ...
  // std::vector<std::pair<unsigned, unsigned>>, pair_compare> scclist;
  std::set<unsigned> weak_sccs;
  unsigned num_weak_states = 0;
  std::map<unsigned, unsigned> non_weak_sccs;
  std::priority_queue<std::pair<std::set<unsigned>, unsigned>,
  ...
  if (om_.get(VERBOSE_LEVEL) > 0) {
    std::cout << "The original NBA has been decomposed into " << result.size()
              << " small NBAs\n";
  }
  return result;
}
...
spot::twa_graph_ptr
decomposer::make_twa_with_scc(spot::scc_info &si, std::set<unsigned> sccs,
  ...
  for (auto &t : nba_->edges()) {
    ...
    if (is_good_scc(scc_src) && is_good_scc(scc_dst)) {
      //   if(sccs.find(scc_src) != sccs.end() && sccs.find(scc_dst) !=
      //   sccs.end())
