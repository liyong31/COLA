...
unsigned parse_int(const std::string &arg) {
  unsigned result;
...
enum output_aut_type { Generic = 0, Buchi, Rabin, Parity };

spot::twa_graph_ptr to_deterministic(spot::twa_graph_ptr aut,
                                     spot::option_map &om, unsigned aut_type,
                                     determinize_t algo) {
  // determinization, only have one determinization algorithm
  spot::twa_graph_ptr res = cola::determinize_tnba(aut, om);
  return res;
}

spot::twa_graph_ptr to_tba(spot::twa_graph_ptr aut) {
...
int main(int argc, char *argv[]) {
  ...
  // Will be deleted
  //  --scc-mem-limit=[INT]
  //          The memory limit (MB) for computing the SCC reachability (default
  //          = 0, no limit)
  //  --scc-num-limit=[INT]
  //          The largest number of SCCs in the deterministic automaton for
  //          merging macrostates (default = 0, no limit)
  ...
  if (contain) {
    ...
    if (parsed_aut->format_errors(std::cerr)) {
      std::runtime_error("File " + file_to_contain +
                         " is not in valid HOA format");
      return 1;
    }
  ...
  for (std::string &path_to_file : path_to_files) {
    if (om.get(VERBOSE_LEVEL))
      std::cout << "File: " << path_to_file << " Algo: " << determinize
                << std::endl;
    ...
    for (;;) {
      ...
      if (print_scc) {
        ...
        std::cout << "Number of IWCs: " << num_iwcs << " with "
                  << num_iwcs_states << " states, in which max IWC with "
                  << num_max_iwcs_states << " states\n";
        std::cout << "Number of ACC_IWCs: " << num_acc_iwcs << " with "
                  << num_acciwcs_states << " states, in which max IWC with "
                  << num_max_acciwcs_states << " states\n";
        std::cout << "Number of DACs: " << num_dacs << " with "
                  << num_dacs_states << " states, in which max DAC with "
                  << num_max_dacs_states << " states\n";
        std::cout << "Number of NACs: " << num_nacs << " with "
                  << num_nacs_states << " states, in which max NAC with "
                  << num_max_nacs_states << " states\n";
        continue;
      }
      ...
      if (!spot::is_deterministic(aut)) {
        ...
        if (om.get(VERBOSE_LEVEL) >= 2) {
          ...
          std::cout << "Output processed automaton (" << aut->num_states()
                    << ", " << aut->num_edges() << ") to sim_aut.hoa\n";
        }
        clock_t c_end = clock();
        if (om.get(VERBOSE_LEVEL) > 0) {
          std::cout << "Done for preprocessing the input automaton in "
                    << 1000.0 * (c_end - c_start) / CLOCKS_PER_SEC << " ms..."
                    << std::endl;
        }

        if (determinize != NoDeterminize && decompose &&
            aut->acc().is_buchi() && !spot::is_deterministic(aut)) {
          ...
          for (unsigned i = 0; i < subnbas.size(); i++) {
            if (verbose)
              std::cout << "Trying to determinize an NBA with "
                        << subnbas[i]->num_states() << " states\n";
            spot::twa_graph_ptr dpa =
                to_deterministic(subnbas[i], om, aut_type, determinize);
            dpas.push_back(dpa);
          }
          ...
          c_end = clock();
          if (om.get(VERBOSE_LEVEL) > 0) {
            std::cout << "Done for determinizing the input automaton in "
                      << 1000.0 * (c_end - c_start) / CLOCKS_PER_SEC << " ms..."
                      << std::endl;
          }
          aut = res;
      ...
      aut->merge_edges();
      if (om.get(VERBOSE_LEVEL) > 0)
        std::cout << "Number of (states, transitions, colors) in the result "
                     "automaton: ("
                  << aut->num_states() << "," << aut->num_edges() << ","
      ...
      if (post_process != None && !decompose) {
        ...
        if (output_type == Rabin) {
          ...
          // call the alternating cycle decomposition to translate our rabin
          // automaton to parity automaton
        ...
        clock_t c_end = clock();
        if (om.get(VERBOSE_LEVEL) > 0)
          std::cout << "Done for postprocessing the result automaton in "
                    << 1000.0 * (c_end - c_start) / CLOCKS_PER_SEC << " ms..."
                    << std::endl;
      } else if (output_type == Parity) {
        aut = spot::acd_transform(aut);
