...
delayed_simulation::delayed_simulation(const spot::const_twa_graph_ptr nba,
                                       spot::option_map &om)
    : nba_(nba), om_(om) {
    ...
    for (unsigned p = 0; p < num_states_; p++)
      for (unsigned q = 0; q < num_states_; q++)
        if (p == q)
          ...
          if (!win_region_[p][q]) // W is winning for spoiler here, so the
                                  // result is the opposite.
          {
            // std::cout << "(" << states_[p].first << ", " << states_[p].second
            // << ") is simulated by "
            //           << "(" << states_[q].first << ", " << states_[q].second
            //           << ")" << std::endl;
            // twa::prop_set pro;
            ...
            // std::cout << states_[p].first << " is simulated by " <<
            // states_[q].first << std::endl;

            // spot::twa::prop_set pro(false, false, false, false, false,
            // false); spot::twa_graph_ptr p_aut = spot::make_twa_graph(nba_,
            // pro); p_aut->set_init_state(states_[p].first);
            ...
            // std::cout << "Correctness = " << !p_aut->intersects(qaut_c) <<
            // std::endl;
          }
        }
  }
...
void delayed_simulation::get_avoid_set(bool_mat &avoid) {
...
bool delayed_simulation::get_winning_set(bool_mat &avoid) {
  bool changed = false;
  ...
  for (unsigned p = 0; p < num_states_; p++)
    for (unsigned q = 0; q < num_states_; q++) {
      ...
      if (win_region_[p][q])
        continue;
      // p does not do more than q, but for some edge p has accepting trans,
      // while q does not isFinal(p) && avoid[p][q] immediate winning, if p
      // reach p or
...
bool delayed_simulation::back_reach(unsigned p, unsigned q, bool_mat &imm_win) {
  bool trapped = false;
