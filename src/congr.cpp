...
namespace cola
{
        ...
        : om_(om), A_(A), B_(B), si_B_(si_B), si_A_(si_A), simulator_(B, si_B, implications, om.get(USE_SIMULATION) > 0)
    {
        ...
        {
            prefix_map.emplace(i, std::set<prefix_graph>());
    ...
    void congr::compute_prefix_simulation()
    {
        ...
        prefix_map[init_A].insert(init_val); 
        ...
        while (! todo.empty())
        {
            ...
            for (auto& tr_A : A_->out(s))
            {
				...
				// copy this set
				std::set<prefix_graph> copy = prefix_map[s];
				for (const prefix_graph& gr : copy)
				{
					...
					while (all != bddfalse)
					{
						...
						std::set<prefix_graph> orig = prefix_map[t];
	...
	void congr::add_to_minimized_period_set(std::set<arc>& set, arc& triple) 
	{
		...
		while (it1 != set.end())
		{
			...
			// q is simulated by exiting state
			arc tpl = *old_it1;
			if (tpl.src_ == triple.src_ && (!tpl.acc_ || triple.acc_)
					&& simulator_.simulate(triple.dst_, tpl.dst_)) 
			{
				// some triple in set can be simulated by triple
				it1 = set.erase(old_it1);
			}else if (tpl.src_ == triple.src_ && (tpl.acc_ || !triple.acc_)
					&& simulator_.simulate(tpl.dst_, triple.dst_))
			{
	...
	bool congr::is_simulated(const period_graph& left, const period_graph& right) 
	{
		for (const arc& lft : left.repr_) 
		{
			...
			{
				if (lft.src_ == rgt.src_ && simulator_.simulate(rgt.dst_, lft.dst_)
					&& (! lft.acc_ || rgt.acc_)) 
				{
					simulated = true;
					break;
    ...
    void congr::compute_period_simulation(unsigned acc_state, const prefix_graph&  simulated_graph)
    {
				...
				while (all != bddfalse)
				{
					...
					for (unsigned p : simulated_set)
					{
						for (auto & tr_B : B_->out(p)) 
						{
							...
							if (si_B_.scc_of(p) == si_B_.scc_of(tr_B.dst)
							&& si_B_.is_accepting_scc(si_B_.scc_of(p)))
							{
								// add arc p->tr_B.dst
					...
					update.word_.emplace_back(letter);
					// check update
					std::set<period_graph> & graphs = period_map[t]; 
					if (can_add_to_period_set(graphs, update))
					{
						...
						{
							// decide whether it ...
							for (auto& pref : prefix_map[t]) 
							{
								if (is_empty(pref.repr_, update)) 
								{
		...
		while (! todo.empty()) 
		{
			...
			for (auto& tr_A : A_->out(s))
            {
				...
				bdd letters = tr_A.cond;
				// copy this set
				std::set<period_graph> copy = period_map[s];
					...
					while (all != bddfalse)
					{
						...
						for (const arc& curr_tpl : gr.repr_)
						{
							...
							for (auto & tr_B : B_->out(curr_tpl.dst_)) 
							{
								...
								{
									// add arc
									arc tpl = arc(p, tr_B.dst, tr_B.acc.has(0) || curr_tpl.acc_);
									add_to_minimized_period_set(update.repr_, tpl);
								}
								// reachable states will be kept simply as (q, false, q)
								reached_states.insert(tr_B.dst);
						...
						if (can_add_to_period_set(graphs, update))
						{
							...
							{
								// decide whether it ...
								for (const prefix_graph& pref : prefix_map[acc_state]) 
								{
									if (is_empty(pref.repr_, update)) 
									{
	...
	bool congr::is_empty(const state_set& prefix, const period_graph& period)
	{
		...
		for(const arc& from: period.repr_) 
		{
			...
			for(const arc& to : period.repr_) 
			{
				...
				{
					nba->new_acc_edge(s, t, p1, from.acc_);
				}
			}
		}
	...
	void congr::contains()
	{
		...
		for (unsigned acc_state : acc_states)
		{
			...
			std::set<prefix_graph>& prefixes = prefix_map[acc_state];
			if (prefixes.empty()) 
			{
				continue;
			}
    ...
    congr_contain(spot::twa_graph_ptr B, spot::twa_graph_ptr A, spot::option_map& om)
    {
       ...
       congr c(om, B, si_B, A, si_A, implications);
       c.contains();
    }
