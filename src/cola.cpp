...
bool is_elevator_automaton(const spot::const_twa_graph_ptr &aut) {
  ...
  for (unsigned scc = 0; scc < nc; ++scc) {
    if (is_deterministic_scc(scc, si) ||
...
std::vector<bool> get_deterministic_sccs(spot::scc_info &si) {
  std::vector<bool> res;
  unsigned nc = si.scc_count();
  for (unsigned scc = 0; scc < nc; ++scc) {
...
std::string get_set_string(const state_set &set) {
  std::string res = "{";
  bool first = true;
  for (state_t s : set) {
    if (first) {
...
std::vector<bool> find_scc_paths(const spot::scc_info &scc) {
  ...
  for (unsigned i = 0; i < scccount; ++i) {
    std::cout << "i = " << i << " count = " << scccount
              << " index = " << (i + scccount * i) << std::endl;
    // reach itself
...
//     }
//     if (is_deterministic_scc(scc, si) || spot::is_inherently_weak_scc(si,
//     scc))
//       continue;
//     return false;
//   }
// }
std::vector<bool> get_accepting_reachable_sccs(spot::scc_info &si) {
...
bool is_limit_deterministic_automaton(const spot::scc_info &si,
                                      std::string &scc_str) {
  unsigned nscc = si.scc_count();
  assert(nscc);
  std::vector<bool> reachable_from_acc(nscc);
...
void get_reachable_sccs(const spot::scc_info &si,
...
std::string get_scc_types(spot::scc_info &si, bool prefer_dac) {
  ...
  for (unsigned sc = 0; sc < nc; ++sc) {
    char type = 0;
    type |= is_deterministic_scc(sc, si)
                ? SCC_INSIDE_DET_TYPE
                : 0; // only care about the states inside SCC
    type |= is_deterministic_scc(sc, si, false)
                ? SCC_DET_TYPE
                : 0; // must also be deterministic for all transitions after
                     // accepting
...
// copied from siminator/cutdet.cpp, this function is similar to
// spot/is_det.cpp/ about semi-deterministic..
bool is_deterministic_scc(unsigned scc, spot::scc_info &si, bool inside_only) {
