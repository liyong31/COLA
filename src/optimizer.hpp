...
struct state_set_hash {
  size_t operator()(const state_set &s) const noexcept {
    size_t hash = 0;
    for (const auto &p : s) {
...
typedef std::unordered_map<state_set, state_set, state_set_hash>
    mstate_equiv_map;
...
private:
  ...
  // Item = (set of reachable states of NBA, the set of mstates with the set of
  // reachable states).
...
public:
  state_simulator(const spot::const_twa_graph_ptr &nba, spot::scc_info &si,
                  std::vector<bdd> &implications, bool use_simulation = true);
...
public:
  edge_strengther(spot::const_twa_graph_ptr nba, const spot::scc_info &si,
                  unsigned threshold);
  ...
  virtual bool cycle_found(unsigned start) override {
    // from start
    ...
    if (is_acc) {
      ...
      if (overlap_initialized) {
        edge_set common;
        std::set_intersection(ts.begin(), ts.end(), overlap_.begin(),
                              overlap_.end(),
